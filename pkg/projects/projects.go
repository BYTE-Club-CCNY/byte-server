...
func get(c *fiber.Ctx) error {
	...
	var projects []getProjectsReturn
	query := database.DB.Scopes(database.Paginate(params.Page))

	query.Table("projects.project AS p").
		Select(`p.uuid, p.name, p.topic, p.short_desc, p.long_desc, 
				p.github, p.image, p.tech_stack,
		...
		Joins("INNER JOIN projects.team t ON p.uuid = t.uuid").
		Joins("LEFT JOIN users.user u1 ON t.member1 = u1.uuid").
		Joins("LEFT JOIN users.user u2 ON t.member2 = u2.uuid").
		Joins("LEFT JOIN users.user u3 ON t.member3 = u3.uuid").
		Joins("LEFT JOIN users.user u4 ON t.member4 = u4.uuid").
